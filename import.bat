title GoAnimate Wrapper Import Script
...
:error_location
echo Doesn't seem like this script is in GoAnimate Wrapper's utilities folder.
goto end
:noerror_location
...
if !folderfilled!==n (
	...
	if not exist "!CFDIR!" echo  This File Does Not exist & goto reaskforfile
	echo:
...
for %%a in (import_these\*) do (
	...
	if !cfext!==.wav set CFTYPE="sound"
	if !cfext!==.ogg set CFTYPE="sound"
	:: Error catch
